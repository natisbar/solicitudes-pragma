        ...
        return usuarioGateway.obtenerPorListaCorreos(obtenerListaCorreos(listaSolicitudes))
                ...
                .flatMapMany(listaUsuarios -> Flux.fromIterable(listaSolicitudes)
                        .flatMap(solicitudPrestamo -> solicitudPrestamoGateway
                                .obtenerDeudaTotalMensualSolicitudesAprobadas(solicitudPrestamo.getCorreo())
                                .map(deudaMensualTotal -> solicitudPrestamo.toBuilder()
                                        .solicitante(listaUsuarios.stream()
                                                .filter(usuario -> usuario.getCorreoElectronico()
                                                .equals(solicitudPrestamo.getCorreo()))
                                                .findFirst().orElse(null))
                                        .deudaTotalMensualSolicitudesAprobadas(deudaMensualTotal)
                                        .build()))
                        )
                .collectList()
